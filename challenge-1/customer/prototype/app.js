...
const i18n = {
  ar: {
    ...
    "public.audienceIndividuals": "الأفراد: مواطنو الدولة، المقيمون، كبار المواطنين، وأصحاب الهمم.",
    "public.audienceWomen": "فئات النساء المشمولة بالخدمة حسب بطاقة الخدمة الرسمية.",
    ...
    "field.propertyAddress": "عنوان العقار السكني",
  ...
  en: {
    ...
    "public.audienceIndividuals": "Individuals (UAE National, Resident, Senior UAE National, Special Needs)",
    "public.audienceWomen": "Women (widow, divorced, parentless, married to an expatriate, parentage unknown)",
    ...
    "field.propertyAddress": "Housing unit address",
...
const optionText = {
  ...
  documentStatus: {
    ar: [
      ...
      ["wrong_document_type", "نوع مستند خاطئ"],
...
let state = {
  ...
  pendingUploadSource: null  // set by label click before file picker opens; "upload" | "camera"
};
...
function init() {
  state.selectedCase = resetCustomerEnteredFields(mockCases[0] ? structuredClone(mockCases[0]) : {});
  ...
  const currentApplicationId = state.applicantMemory?.currentApplicationId || state.selectedCase.id;
  const existing = findLatestSubmissionForAccount(currentApplicationId);
  if (existing) state.selectedCase = { ...state.selectedCase, ...existing };
  if (existing?.status) state.status = existing.status;
  if (existing) state.lastSubmission = existing;
  state.applicationActive = Boolean(isCustomerSignedIn() && (state.applicantMemory?.currentApplicationId || existing));
  bindEvents();
  applyLanguage();
  fillForm(state.selectedCase);
  renderAll();
  // Refresh status from server if returning to a live-backed application
  if (state.selectedCase?.liveApiBacked) {
    window.setTimeout(() => refreshLiveStatus().catch(() => {}), 400);
  }
}
...
function bindEvents() {
  ...
  [
    ["incomeDocumentFile", "incomeDocumentFile", "income_proof"],
    ["missionLetterFile",  "missionLetterFile",  "official_mission_letter"],
    ["passportStampFile",  "passportStampFile",  "passport_stamp"]
  ].forEach(([labelId, inputId, slot]) => {
    // Track whether the user triggered upload via camera or file picker.
    // On mobile with capture="environment" the browser handles the choice;
    // we detect camera intent when a label marked data-upload-source="camera" is clicked.
    const labelEl = document.querySelector(`label[for="${labelId}"]`);
    if (labelEl) {
      labelEl.addEventListener("click", () => {
        state.pendingUploadSource = labelEl.dataset.uploadSource || "upload";
      });
    }
  ...
  [
    ["incomeDocumentClear", "incomeDocumentFile", "income_proof"],
    ["missionLetterClear", "missionLetterFile", "official_mission_letter"],
    ["passportStampClear", "passportStampFile", "passport_stamp"]
  ].forEach(([clearId, inputId, slot]) => {
    const btn = $(clearId);
    if (!btn) return;
    btn.addEventListener("click", () => {
      $(inputId).value = "";
      handleDocumentUpload(slot, null);
    });
  });

  $("currentSalary").addEventListener("input", () => {
    validateSalaryField();
    syncFormToState();
    updateCustomerDocumentGate();
    renderSteps();
  ...
  $("currentSalary").addEventListener("blur", () => validateSalaryField());
...
function readApplicantMemory() {
  ...
  try {
    return JSON.parse(localStorage.getItem(APPLICANT_MEMORY_FALLBACK_KEY) || "null");
  } catch {
    return null;
  }
}
...
function saveApplicantMemory(memory) {
  ...
  localStorage.setItem(APPLICANT_MEMORY_FALLBACK_KEY, JSON.stringify(next));
  return next;
}
...
function makeApplicationId() {
  ...
  const existingIds = new Set(readSubmissions().map((item) => item.id));
  const randomPart = () => {
  ...
  let id = "";
  do {
    id = `MOEI-HOUSING-${year}-${Date.now().toString(36).toUpperCase()}-${randomPart()}`;
...
function createApplicationShell() {
  ...
  return {
    ...resetCustomerEnteredFields(mockCases[0] ? structuredClone(mockCases[0]) : {}),
    id: makeApplicationId(),
...
function accountSubmissions() {
  const applicantId = state.applicantMemory?.applicantId || state.verifiedSession?.subjectId;
  if (!applicantId) return [];
  return readSubmissions().filter((item) => item.applicantMemoryId === applicantId);
}

function findLatestSubmissionForAccount(preferredId = null) {
  const submissions = accountSubmissions();
  if (preferredId) {
    const preferred = submissions.find((item) => item.id === preferredId);
...
function openApplication(mode) {
  ...
  const latest = findLatestSubmissionForAccount(state.applicantMemory.currentApplicationId);

  if (mode === "resume" && latest) {
    state.selectedCase = mergeApplicantMemoryIntoCase({ ...resetCustomerEnteredFields(mockCases[0] ? structuredClone(mockCases[0]) : {}), ...latest }, state.applicantMemory);
    state.status = latest.status || "draft";
    state.lastSubmission = latest;
  } else {
  ...
  rememberCurrentApplication(state.selectedCase.id);
  fillForm(state.selectedCase);
  renderAll();
...
function localDocumentRecognitionFallback(request) {
  ...
  const LOCAL_DOC_LABELS = {
    salary_certificate: ["Salary certificate", "شهادة الراتب"],
    non_work_letter: ["Non-work letter", "رسالة عدم العمل"],
    official_mission_letter: ["Official mission letter", "شهادة المهمة الرسمية"],
    passport_stamp: ["Passport stamp", "ختم جواز السفر"]
  };
  let detectedWrongType = null;
  for (const [typeKey, pattern] of Object.entries(LOCAL_TYPE_SIGNALS)) {
    if (typeKey !== documentKey && pattern.test(fileName)) {
      detectedWrongType = typeKey;
      break;
    }
  }
  if (detectedWrongType) {
    const expectedLabel = LOCAL_DOC_LABELS[documentKey] || [documentKey, documentKey];
    const detectedLabel = LOCAL_DOC_LABELS[detectedWrongType] || [detectedWrongType, detectedWrongType];
    return {
      contractVersion: "document-recognition-result.v1",
      ...
      document: {
        ...
        checks: [
          ...
          { id: "document_type_match", status: "fail", label: `Correct document type (expected: ${expectedLabel[0]})`, arLabel: `نوع المستند صحيح (المطلوب: ${expectedLabel[1]})` }
        ],
        customerMessage: `Wrong document type. Expected: ${expectedLabel[0]}. Filename suggests: ${detectedLabel[0]}. Upload the correct document.`,
        customerMessageAr: `نوع المستند غير صحيح. المطلوب: ${expectedLabel[1]}. المستند المرفوع يبدو: ${detectedLabel[1]}. ارفع المستند الصحيح.`
      }
    };
  }
  ...
  const stamped = !missingStampSignal && (documentKey === "passport_stamp" || /(stamp|stamped|digital|seal|sealed|notary|signed|official)/i.test(fileName));
  const customerStatus = unreadable
    ? "unreadable"
    : dateMismatch
      ? "date_mismatch"
      : salaryConflict
...
function persistWorkspaceState(reason = "workspace_update") {
  ...
  const workspaceCase = {
    ...
    channel: state.selectedCase.channel || "customer-prototype",
    source: state.selectedCase.source || "customer"
  };
  if (workflow?.saveWorkspaceState) {
    const saved = workflow.saveWorkspaceState(workspaceCase, {
      status: workspaceCase.status,
      language: state.lang,
      channel: "customer-prototype",
      source: "customer"
    });
    state.selectedCase = { ...state.selectedCase, ...saved };
    if (state.lastSubmission?.id === saved.id) state.lastSubmission = saved;
    return saved;
  }
...
function renderSelectedFileNames() {
  [
    ["incomeDocumentFileName", "incomeDocumentClear", "income_proof"],
    ["missionLetterFileName", "missionLetterClear", "official_mission_letter"],
    ["passportStampFileName", "passportStampClear", "passport_stamp"]
  ].forEach(([nameId, clearId, slot]) => {
    const node = $(nameId);
    const clearBtn = $(clearId);
    if (!node) return;
    const fileName = state.selectedCase.documentUploads?.[slot]?.file?.name;
    node.textContent = fileName || t("docs.noFile");
    if (clearBtn) clearBtn.hidden = !fileName;
  });
}

/* ── Address autocomplete ────────────────────────────────────────────── */
(function initLocationAutocomplete() {
  const inputEl    = document.getElementById("propertyAddress");
  const dropEl     = document.getElementById("propertyAddressDropdown");
  const spinnerEl  = document.getElementById("propertyAddressSpinner");
  const checkEl    = document.getElementById("propertyAddressCheck");
  ...
  function showDropdown(items) {
    ...
    dropEl.innerHTML = items.map((r, i) =>
      `<button type="button" class="location-option" data-idx="${i}" data-label="${r.label.replace(/"/g,"&quot;")}">
        <span class="location-pin">📍</span><span>${r.label}</span>
      </button>`
    ).join("");
    dropEl.hidden = false;
  }

  function hideDropdown() { dropEl.hidden = true; dropEl.innerHTML = ""; }
  ...
  inputEl.addEventListener("input", () => {
    ...
    debounceTimer = setTimeout(() => search(q), 420);
  });
...
function fillForm(caseData) {
  ...
  const addrEl = $("propertyAddress");
  if (addrEl) addrEl.value = caseData.propertyAddress || "";
...
function validateSalaryField() {
  const input = $("currentSalary");
  const errorEl = $("salaryError");
  ...
  if (isNaN(value) || value <= 0) {
    ...
    errorEl.textContent = state.lang === "ar"
      ? "يرجى إدخال مبلغ راتب صحيح وموجب"
      : "Please enter a valid positive salary amount";
    errorEl.hidden = false;
  } else if (value < 1000) {
    input.classList.add("field-error");
    errorEl.textContent = state.lang === "ar"
      ? "الراتب الأدنى المقبول 1,000 درهم"
      : "Minimum accepted salary is AED 1,000";
    errorEl.hidden = false;
  } else if (value > 500000) {
    input.classList.add("field-error");
    errorEl.textContent = state.lang === "ar"
      ? "الراتب الأقصى المقبول 500,000 درهم"
      : "Maximum accepted salary is AED 500,000";
    errorEl.hidden = false;
  } else {
    input.classList.remove("field-error");
    errorEl.hidden = true;
...
function syncFormToState() {
  ...
  state.selectedCase = {
    ...state.selectedCase,
    propertyAddress: ($("propertyAddress")?.value || state.selectedCase.propertyAddress || ""),
...
function buildLiveApplicationPayload() {
  ...
  return {
    ...
    propertyAddress: state.selectedCase.propertyAddress || "",
    acknowledgement: Boolean(state.selectedCase.acknowledgement)
  };
}

// 7 MB original → ~9.3 MB base64, safely under server MAX_BASE64_BYTES (10 MB)
const MAX_CLIENT_BYTES = 7 * 1024 * 1024;
...
async function buildLiveDocumentPayload(upload) {
  const base = {
  ...
  // Source is set by the label click handler before the file picker opens (see bindEvents).
  // Defaults to "upload" — never assume camera unless the state flag was explicitly set.
  const source = (state.pendingUploadSource) || "upload";
  state.pendingUploadSource = null; // consume once
  base.source   = source;
  base.mimeType = file.type;

  const IMAGE_COMPRESS_THRESHOLD = 2 * 1024 * 1024; // compress images > 2 MB
  ...
  try {
    ...
    if (processedFile.size > MAX_CLIENT_BYTES) {
      base.uploadError = "file_too_large";
      return base;
    }
    base.fileBase64 = await readFileAsBase64(processedFile);
  } catch (err) {
    console.warn("[doc-payload] base64 read failed:", err);
  }
...
async function saveLiveSubmission() {
  if (!liveApi?.createApplication) return saveSubmission("system_review");
  const created = await liveApi.createApplication(buildLiveApplicationPayload());
  const application = created.application;
  const applicationId = application.applicationId;
  const uploadEntries = Object.entries(state.selectedCase.documentUploads || {}).filter(([, u]) => Boolean(u));
  for (const [slotKey, upload] of uploadEntries) {
    const payload = await buildLiveDocumentPayload({ ...upload, slotKey });
    const docResult = await liveApi.addDocument(applicationId, payload).catch(() => null);

    // Surface AI-detected wrong document type back to the slot so the correction notice shows it
    const blockingIssues = docResult?.document?.intelligence?.blockingIssues || [];
    if (blockingIssues.includes("wrong_document_type")) {
      state.selectedCase.documentUploads = {
        ...state.selectedCase.documentUploads,
        [slotKey]: {
          ...upload,
  ...
  // Persist live submission to localStorage so the Track Request panel survives a page refresh
...
function saveSubmission(status) {
  if (workflow) {
    const actionMap = {
      draft: ["Draft saved from customer portal", "تم حفظ المسودة من بوابة المتعامل"],
      system_review: ["Application passed customer gateway checks and was submitted for review", "اجتاز الطلب فحص البوابة وتم إرساله للمراجعة"]
    };
    const [action, actionAr] = actionMap[status] || actionMap.draft;
    const submission = workflow.saveSubmission(state.selectedCase, status, {
      language: state.lang,
      channel: "customer-prototype",
      source: "customer",
      actor: "customer",
    ...
    state.lastSubmission = submission;
    rememberCurrentApplication(submission.id);
    return submission;
  }
  ...
  const submission = {
    ...
    channel: "customer-prototype",
    handoffTarget: "office-prototype"
  };

  const existing = readSubmissions().filter((item) => item.id !== submission.id);
...
function readSubmissions() {
  if (workflow) return workflow.readSubmissions();
  try {
...
function persistSubmission(submission) {
  if (workflow) return; // workflow manages its own store
  try {
    const all = readSubmissions().filter((item) => item.id !== submission.id);
...
async function refreshLiveStatus() {
  if (!liveApi?.getApplication) return;
  const applicationId = state.selectedCase?.applicationId || state.selectedCase?.id;
  if (!applicationId || !state.selectedCase?.liveApiBacked) return;
  try {
    const { application } = await liveApi.getApplication(applicationId);
    if (!application) return;
    const refreshed = { ...state.lastSubmission, ...application, id: applicationId, applicationId, liveApiBacked: true };
    state.lastSubmission = refreshed;
    state.status = application.status || state.status;
    persistSubmission(refreshed);
    renderAll();
  } catch { /* server offline — keep showing last known status */ }
}
...
function statusLabel(status) {
  const labels = {
    ...
    wrong_document_type: { ar: "نوع مستند خاطئ", en: "Wrong document type" },
...
function documentCorrectionMessage(doc) {
  ...
  if (doc.status === "wrong_document_type") {
    return state.lang === "ar"
      ? "نوع المستند غير صحيح — ارفع المستند المطلوب في هذه الخانة."
      : "Wrong document type — upload the correct document for this slot.";
  }
  return state.lang === "ar"
...
function buildCustomerReadiness(caseData) {
  ...
  } else if (currentSalary < 1000 || currentSalary > 500000) {
    issues.push({
      key: "currentSalary",
      text: state.lang === "ar"
        ? "الراتب يجب أن يكون بين 1,000 و500,000 درهم."
        : "Salary must be between AED 1,000 and AED 500,000."
    });
  }
...
function renderDdaConflict() {
  const autoDdaActive = true; // always enabled in this demo — DDA row is hardcoded as "Enabled"
  const checkbox = $("officialMissionCase");
  const row      = $("officialMissionCaseRow");
  const note     = $("ddaConflictNote");
  const missionFields = $("missionDocumentFields");
  if (!checkbox || !note) return;
  if (autoDdaActive) {
    checkbox.disabled = true;
    checkbox.checked  = false;
    if (row) row.classList.add("check-row--disabled");
    if (missionFields) missionFields.hidden = true;
    note.hidden = false;
    note.textContent = state.lang === "ar"
      ? "⚠️ غير قابل للتطبيق — اتفاقية الخصم المباشر نشطة على هذا الحساب."
      : "⚠️ Not applicable — Direct Debit Agreement is active on this account.";
  } else {
    checkbox.disabled = false;
    if (row) row.classList.remove("check-row--disabled");
    note.hidden = true;
  }
}
...
function currentWizardStep() {
  if (!state.applicationActive) return 1;
  if (state.status !== "draft") return 4; // submitted — all done
  const sal = Number(state.selectedCase.currentSalary || 0);
  const rem = (state.lang === "ar" ? state.selectedCase.remarksAr || state.selectedCase.remarks : state.selectedCase.remarks || state.selectedCase.remarksAr || "").trim();
  const ack = Boolean(state.selectedCase.acknowledgement);
  const step1Done = sal >= 1000 && sal <= 500000 && rem.length > 0 && ack;
  if (!step1Done) return 1;
  const gate = state.selectedCase.customerDocumentGate;
  const step3Done = gate?.ready;
  if (step3Done) return 3;
  return 2;
}

function renderSteps() {
  const active = currentWizardStep();

  const stepListEl = $("stepList");
  if (stepListEl) {
    stepListEl.innerHTML = steps[state.lang]
      .map(([title, copy], index) => {
        const number = index + 1;
        const done = number < active || active > 3;
        const isActive = number === active && active <= 3;
        const className = done ? "step-item done" : isActive ? "step-item active" : "step-item";
  ...
  // Mobile progress bar — only shown when application is open
  const bar = $("mobileStepBar");
  if (bar) {
    bar.hidden = !state.applicationActive;
    if (state.applicationActive) {
      const totalSteps = steps[state.lang].length;
      const clampedActive = Math.min(active, totalSteps);
      const pct = Math.min(100, Math.round(((clampedActive - 1) / totalSteps) * 100));
      bar.innerHTML = `
        <div class="mobile-step-track">
          <div class="mobile-step-fill" style="width:${pct}%"></div>
        </div>
        <span class="mobile-step-label">${steps[state.lang][clampedActive - 1][0]}</span>
        <span class="mobile-step-count">${clampedActive}/${totalSteps}</span>
      `;
    }
  }

  // Highlight form sections matching current step
  document.querySelectorAll(".form-step-section").forEach((fs) => {
    const sNum = Number(fs.dataset.step);
    const badge = fs.querySelector(".step-badge");
    fs.classList.toggle("step-section--active", sNum === active);
    fs.classList.toggle("step-section--done", sNum < active);
    if (badge) badge.textContent = sNum < active ? "✓" : String(sNum);
  });
}
        ...
        <div class="document-check-list">
          ...
          ${checks.map((check) => `
            <p class="check-${check.status}">
              ${check.status === "pass" ? "✅" : "❌"} ${state.lang === "ar" ? escapeHtml(check.arLabel || check.label) : escapeHtml(check.label || check.arLabel)}
            </p>
          `).join("")}
        </div>
      ` : ""}
  ...
  return `
    <div class="document-thread-list">
      ...
      ${visibleAttempts.map((attempt, index) => {
        ...
        const label = index === 0
          ? `${t("docs.historyAttempt")} ${attemptNumber} (${t("docs.historyLatest")})`
          : index === 1
            ? (state.lang === "ar" ? "الرفع السابق 1" : "Previous Upload 1")
            : (state.lang === "ar" ? "الرفع السابق 2" : "Previous Upload 2");
        return `
          <p>
            <strong>${label}</strong>
...
function renderStatus() {
  ...
  $("statusCard").innerHTML = `
    ...
    <p>${handoff}</p>
    ${submission?.updatedAt ? `<p>${updatedLabel}: ${new Date(submission.updatedAt).toLocaleString(state.lang === "ar" ? "ar-AE" : "en-AE")}</p>` : ""}
...
function renderConfirmation() {
  ...
  $("confirmationCard").innerHTML = `
    ...
    <dl>
      ...
      <div><dt>${copy.current}</dt><dd>${citizenStatus}</dd></div>
