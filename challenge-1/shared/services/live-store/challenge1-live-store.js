...
function createChallenge1LiveStore(options = {}) {
  const rootDir = options.rootDir || path.resolve(__dirname, "../../../..");
  const dataDir = options.dataDir || path.join(rootDir, "data", "live", "challenge-1");

  function storePath(storeName) {
    if (!Object.prototype.hasOwnProperty.call(STORE_DEFINITIONS, storeName)) {
      throw new Error(`Unknown Challenge 1 live store: ${storeName}`);
    }
